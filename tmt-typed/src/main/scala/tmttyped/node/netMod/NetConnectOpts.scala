...
object NetConnectOpts {
  ...
  inline def IpcNetConnectOpts(path: String): tmttyped.node.netMod.IpcNetConnectOpts = {
    val __obj = js.Dynamic.literal(path = path.asInstanceOf[js.Any])
    __obj.asInstanceOf[tmttyped.node.netMod.IpcNetConnectOpts]
  }
  
  inline def TcpNetConnectOpts(port: Double): tmttyped.node.netMod.TcpNetConnectOpts = {
