...
import tmttyped.node.nodeStrings.messageerror
import tmttyped.node.nodeStrings.online
import tmttyped.node.nodeUrlMod.URL
import tmttyped.node.perfHooksMod.EventLoopUtilityFunction
import tmttyped.node.perfHooksMod.EventLoopUtilization
...
object workerThreadsMod {
  ...
  /* Rewritten from type alias, can be one of: 
    ...
    - tmttyped.node.workerThreadsMod.MessagePort
    - tmttyped.node.fsPromisesMod.FileHandle
    - tmttyped.node.nodeCryptoMod.X509Certificate
    - tmttyped.node.nodeBufferMod.Blob
  */
