...
object AsyncResource {
  ...
  inline def apply(asyncResource: tmttyped.node.asyncHooksMod.AsyncResource): AsyncResource = {
  ...
  extension [Self <: AsyncResource](x: Self) {
    ...
    inline def setAsyncResource(value: tmttyped.node.asyncHooksMod.AsyncResource): Self = StObject.set(x, "asyncResource", value.asInstanceOf[js.Any])
  }
}
